  ...
  describe '商品出品機能' do
    ...
    context '新規登録がうまくいかないとき' do
      ...
      it 'nameがないと登録できない' do
        @item.name = ''
        @item.valid?
        expect(@item.errors.full_messages).to include("Name must exist"）
      end
      ...
      it 'postage_payes_idが1だと登録できない' do
        ...
        @item.valid?
        expect(@item.errors.full_messages).to include("Postage payes id can't be blank"）
      end
      ...
      it 'handling_tims_idが1だと登録できない' do
        ...
        @item.valid?
        expect(@item.errors.full_messages).to include("Handling tims id can't be blank"）
      end
